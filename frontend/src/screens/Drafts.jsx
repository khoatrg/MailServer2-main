...
const listToRender = sortMessages(rawList, 'date', sortDir);
  ...
  async function deleteDraft(e, m) {
    e.stopPropagation();
    try {
      await deleteDraftMessage(m.mailbox || m.mailbox, m.uid);
      setMessages(prev => prev.filter(x => !(String(x.uid) === String(m.uid) && x.mailbox === m.mailbox)));
    } catch (err) {
      console.warn('deleteDraft failed', err && err.message);
    }
  }
  ...
  <select className="sort-select" value={sortDir} onChange={e => setSortDir(e.target.value)} aria-label="Sort messages">
    ...
    <option value="asc">Oldest first</option>
      ...
      <ul className="message-list">
        {(!paginated || paginated.length === 0) && <li className="empty">{searching ? 'Searching…' : 'No drafts'}</li>}
        {paginated.map(m => {
           ...
           return (
             ...
             <li key={`${m.mailbox}::${m.uid}`} className="message-item" onClick={() => open(m.uid, m.mailbox)}>
               ...
               <div className="message-body">
                 <div className="message-top">
                   <div className="sender">
                     ...
                     <div className="subject">{m.subject ? m.subject : <em style={{color:'#5f6368'}}>(no subject)</em>}</div>
                   </div>
              ...
              <div className="item-actions" onClick={e=>e.stopPropagation()}>
                <button className="list-delete-btn" title="Delete Draft" onClick={(e)=>deleteDraft(e, m)}>🗑</button>
              </div>
             </li>
           );
         })}
